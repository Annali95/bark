...
# class Osc_Plot:
#     def __init__(self,ax,sampled):
#         self.osc_ax = ax
#         self.data = sampled.data[0].ravel()
...
#     def update_oscillogram(self,start,stop):
#         sr = self.sr      
...
class Osc_Plot(Plot):
    def __init__(self,
                 ...
                 sampled,
                 opstack):
        Plot.__init__(self,ax,sampled,opstack)
        self.N_points = 35000
        ...
        self.osc_line, = self.ax.plot(
            ...
            np.zeros(self.N_points),
            color='gray')
        self.osc_boundary_start = self.ax.axvline(color='r')
        self.osc_boundary_stop = self.ax.axvline(color='r')
        self.syl_labels = [self.ax.text(0,
                                            0,
                                            '',
                                            size='xx-large',
    ...
    def update_oscillogram(self,buf_start,buf_stop,i):
        self.selected_boundary = None
        ...
        if self.opstack.ops:
            ...
            last_command = 'none'
        if i == 0:
            self.ax.set_title('ctrl+h for help, prints to terminal')
        else:
            self.ax.set_title('{}/ {} {}'.format(i + 1, len(
...
class Minimap_Plot:
    ...
    def initialize_minimap(self):
        ...
        self.ax.scatter(times,
                            ...
                            edgecolors='none')
        self.ax.vlines(self.opstack.events[self.i]['start'],
                           -1,
                           38,
                           zorder=0.5,
                           color='w',
        ...
        self.ax.tick_params(axis='y',
                                ...
                                right='off',
                                labelleft='off')
        self.ax.set_ylim(-1, 38)
    def update_minimap(self,i):
...
class SegmentReviewer:
    def __init__(self,
        ...
        self.map = Minimap_Plot(ax = map_ax, opstack = opstack)
    ...
    def update_plot_data(self):
        ...
        self.map.update_minimap(self.i)       
        ...
        self.osc.update_oscillogram(self.buffer_start_samp,self.buffer_stop_samp,self.i)
        self.canvas.draw()
    ...
    def on_mouse_press(self, event):
        if event.inaxes in (None, self.spec.ax) or event.button != 1:
            return
        start_pos = self.osc.osc_boundary_start.get_xdata()[0]
        stop_pos = self.osc.osc_boundary_stop.get_xdata()[0]
        ...
        # boundary updates
            ...
            xlim1, xlim2 = self.osc.ax.get_xlim()
            xspan = xlim2 - xlim1
            if abs(event.xdata - start_pos) / xspan < 0.007:
                self.osc.selected_boundary = self.osc.osc_boundary_start
            elif abs(event.xdata - stop_pos) / xspan < 0.007:
                self.osc.selected_boundary = self.osc.osc_boundary_stop
            if self.osc.selected_boundary:
                self.osc.selected_boundary.set_color('y')
                self.canvas.draw()

    def on_mouse_motion(self, event):
        if self.osc.selected_boundary is None:
            return
        self.osc.selected_boundary.set_xdata((event.xdata, event.xdata))
        self.canvas.draw()

    def on_mouse_release(self, event):
        if self.osc.selected_boundary == self.osc.osc_boundary_start:
            self.opstack.push(Update(self.i, 'start', float(event.xdata)))
        elif self.osc.selected_boundary == self.osc.osc_boundary_stop:
            self.opstack.push(Update(self.i, 'stop', float(event.xdata)))
        if self.osc.selected_boundary:
            self.osc.selected_boundary.set_color('r')
            self.osc.update_syl_boundaries()
        self.osc.selected_boundary = None
        self.canvas.draw()
